...
if [ "$1" = "--check" ]; then
  ...
  while IFS= read -r f; do
    total=$((total + 1))
    if grep -qE '<header id="site-header" class="site-header[^"]*"></header>' "$f" 2>/dev/null; then
      count=$((count + 1))
      echo "  $f"
    fi
...
# Replace ANY existing header content (empty OR stale) with the canonical header.
while IFS= read -r f; do
  total=$((total + 1))
...
if grep -qE '<header id="site-header" class="site-header[^"]*">' 404.html 2>/dev/null; then
  sed -i '' -E "s|<header id=\"site-header\" class=\"(site-header[^\"]*)\">.*</header>|<header id=\"site-header\" class=\"\1\">${ESCAPED_HEADER}</header>|" 404.html
  count=$((count + 1))
fi
