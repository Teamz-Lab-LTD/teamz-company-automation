...
while IFS= read -r f; do
  total=$((total + 1))
  if grep -q '<header id="site-header" class="site-header">' "$f" 2>/dev/null; then
...
if grep -q '<header id="site-header" class="site-header">' 404.html 2>/dev/null; then
